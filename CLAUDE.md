...
- **v1.6.0 defaults** (data-driven; delivered via `migrate_config` so explicit user values survive): `lot_sizing.mode` defaults to `total_lot` for new installs (existing users keep their mode); `lot_sizing.skip_limits_at` (default 6, 0 = off) skips signals carrying ≥ N limits at placement — existing pendings/fills untouched, gated in `sync_cycle` after the unmanaged filter; `partial_close_percent` defaults to **0 = trail the full position** (trailing exits beat fixed TP on real outcome data) — the migration flips only values still at the old shipped defaults (50/75). `POST /api/config/reset` restores the bundled template while preserving license/terminal-path/symbol wiring ("Restore defaults" button in Settings).
